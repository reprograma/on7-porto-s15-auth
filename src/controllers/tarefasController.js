...
const getAll = (req, res) => {
  ...
  const token = authHeader.split("")[1]; jwt.verify(token, SECRET, function (erro) {
    if (erro) {
...
const deleteTarefa = (req, res) => {
  ...
  if (!authHeader) {
    ...
    const token = authHeader.split(" ")[1]; jwt.verify(token, SECRET, (err) => {
    if (err) {
      return res.status(403).send("Token inválido");
    };
    tarefas.find({id,}, function (err, tarefa) {
        if (tarefa.length > 0) { tarefas.deleteMany({id,},
            function (err) {
              if (err) {
                return res.status(500).send({ message: err.message, status: "FAIL",});
              };
              return res.status(200).send({message: "Tarefa removida com sucesso", status: "SUCCESS",});
            });
          } else {
          return res.status(200).send({ message: "Não há tafera para ser removida", status: "EMPTY",
          });
      };
    });
  });
...
const deleteTarefaConcluida = (req, res) => {
  ...
  jwt.verify(token, SECRET, err => {
    ...
    tarefas.find({ concluido: true }, (err, tarefa) => {
      if (tarefa.length > 0) {tarefas.deleteMany({ concluido: true }, err => {
          if (err) {
...
const putTarefa = (req, res) => {
  ...
  const token = authHeader.split(" ")[1]; jwt.verify(token, SECRET, (err) => {
    ...
    tarefas.find({id,},function (err, tarefa) {
        if (tarefa.length > 0) {tarefas.updateMany({id,}, {$set: req.body,},
            function (err) {
              if (err) {
